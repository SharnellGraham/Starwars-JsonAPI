...
// get starship by hyperdrive Rating 
starshipRouter.get('/hyperdrive/:hyperdrive_rating', async (req, res) => {
  try {
    const starship = await Starship.findOne({ hyperdrive_rating: req.params.hyperdrive_rating })
    if (!starship) {
      throw new Error('Starship not found')
    }
    res.json(starship)
